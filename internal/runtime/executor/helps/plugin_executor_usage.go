...
func ParsePluginExecutorResponseUsage(protocol string, payload []byte) usage.Detail {
	...
	case "gemini":
		detail, _ := ParseGeminiUsage(payload)
		return detail
	case "interactions", "interactions-response":
		detail, _ := ParseInteractionsUsage(payload)
		return detail
	case "antigravity":
		detail, _ := ParseAntigravityUsage(payload)
		return detail
	case "codex", "openai-response":
		if detail, ok := ParseCodexUsage(payload); ok {
			return detail
		}
		detail, _ := ParseOpenAIUsage(payload)
		return detail
	default:
		detail, _ := ParseOpenAIUsage(payload)
		return detail
...
func parseClaudePayloadUsage(payload []byte) usage.Detail {
	...
	detail, _ := ParseClaudeUsage([]byte(`{"usage":` + usageNode.Raw + `}`))
	return detail
}
...
func parseClaudeStreamLine(line []byte) (usage.Detail, bool) {
	...
	detail, observed := ParseClaudeUsage([]byte(`{"usage":` + usageNode.Raw + `}`))
