		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				for scanner.Scan() {
					...
					// Capture provider usage before filtering non-terminal metadata from
					// the downstream stream.
					if rawPayload := helps.JSONPayload(line); rawPayload != nil {
						if detail, ok := helps.ParseAntigravityStreamUsage(rawPayload); ok {
							reporter.PublishObserved(ctx, detail)
						}
					}
