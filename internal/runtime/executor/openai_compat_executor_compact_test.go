...
func TestOpenAICompatExecutorMultiLineSSEUsagePublishesFinalRecord(t *testing.T) {
	const model = "openai-compat-multiline-sse-usage-test"
	plugin := &captureOpenAICompatUsagePlugin{records: make(chan usage.Record, 2)}
	usage.RegisterNamedPlugin("openai-compat-multiline-sse-usage-test", plugin)

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"id\":\"chatcmpl_1\",\"object\":\"chat.completion.chunk\",\"choices\":[],\"usage\":\n"))
		_, _ = w.Write([]byte("data: {\"prompt_tokens\":2,\"completion_tokens\":3,\"total_tokens\":5}}\n\n"))
		_, _ = w.Write([]byte("data: [DONE]\n\n"))
	}))
	defer server.Close()
