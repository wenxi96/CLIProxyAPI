...
func TestPrepareAntigravityGeminiReasoningReplayPayloadAppendsStaleThoughtSignatureWithoutNullParts(t *testing.T) {
	...
	item := []byte(`{"type":"thought_signature","contentIndex":8,"partIndex":3,"thoughtSignature":"sig-text-valid-123"}`)
	...
	if got := parts[1].Get("thoughtSignature").String(); got != "sig-text-valid-123" {
		t.Fatalf("thoughtSignature = %q, want sig-text-valid-123; body=%s", got, out)
	}
}
