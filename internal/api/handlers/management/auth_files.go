...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	return h.buildAuthFileEntryWithStatus(auth, coreauth.PoolAuthSnapshot{}, nil)
}

func (h *Handler) buildAuthFileEntryWithStatus(auth *coreauth.Auth, poolStatus coreauth.PoolAuthSnapshot, authUsage map[string]usage.AuthUsageSnapshot) gin.H {
	authFileEntryMu.Lock()
	defer authFileEntryMu.Unlock()
	return h.buildAuthFileEntryLocked(auth, poolStatus, authUsage)
}

func (h *Handler) buildAuthFileEntryLocked(auth *coreauth.Auth, poolStatus coreauth.PoolAuthSnapshot, authUsage map[string]usage.AuthUsageSnapshot) gin.H {
	if auth == nil {
		return nil
	}
	auth.EnsureIndex()
