...
func (h *Handler) readAuthFileForDownload(name string) ([]byte, string, error) {
	...
	if errValidate := validateAuthFileDownloadName(normalizedName); errValidate != nil {
		return nil, normalizedName, errValidate
	}
	...
	if errRead != nil {
		...
		return nil, normalizedName, fmt.Errorf("failed to read file: %w", errRead)
	}
...
// DownloadAuthFilesArchive downloads multiple auth files as a single ZIP archive.
func (h *Handler) DownloadAuthFilesArchive(c *gin.Context) {
	...
	for _, name := range names {
		data, normalizedName, errRead := h.readAuthFileForDownload(name)
		if errRead != nil {
			_ = zipWriter.Close()
			writeAuthFileDownloadError(c, errRead)
			return
		}

		entryWriter, errCreate := zipWriter.Create(normalizedName)
		if errCreate != nil {
			_ = zipWriter.Close()
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to create zip entry: %v", errCreate)})
			return
		}
		if _, errWrite := entryWriter.Write(data); errWrite != nil {
			_ = zipWriter.Close()
	...
	if errClose := zipWriter.Close(); errClose != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to finalize zip archive: %v", errClose)})
		return
	}
