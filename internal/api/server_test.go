...
func newTestServerWithOptions(t *testing.T, opts ...ServerOption) *Server {
	...
	prevUsageEnabled := internalusage.StatisticsEnabled()
	prevQueueUsageEnabled := redisqueue.UsageStatisticsEnabled()
	t.Cleanup(func() {
		internalusage.SetStatisticsEnabled(prevUsageEnabled)
		redisqueue.SetUsageStatisticsEnabled(prevQueueUsageEnabled)
	})
...
func TestNewServerSynchronizesUsageStatisticsToggle(t *testing.T) {
	server := newTestServer(t)

	if internalusage.StatisticsEnabled() {
		t.Fatal("expected NewServer to disable in-memory usage statistics from config")
	}
	if redisqueue.UsageStatisticsEnabled() {
		t.Fatal("expected NewServer to disable redis usage queue from config")
	}

	nextCfg := *server.cfg
	nextCfg.UsageStatisticsEnabled = true
	server.UpdateClients(&nextCfg)

	if !internalusage.StatisticsEnabled() {
		t.Fatal("expected UpdateClients to enable in-memory usage statistics from config")
	}
	if !redisqueue.UsageStatisticsEnabled() {
		t.Fatal("expected UpdateClients to enable redis usage queue from config")
	}
}
...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	if len(usagePayload.Usage) == 0 || string(usagePayload.Usage) == "null" {
		t.Fatalf("usage response missing usage snapshot: %s", usageRR.Body.String())
	}
