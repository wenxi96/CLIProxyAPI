...
func (s *authScheduler) scopedPoolSnapshot() PoolSnapshot {
	if s == nil || s.scopedPool == nil {
		return PoolSnapshot{GeneratedAt: time.Now(), Strategy: "round-robin", Providers: map[string]PoolProviderSnapshot{}, Auths: map[string]PoolAuthSnapshot{}}
	}
...
func (s *authScheduler) rebuild(auths []*Auth) {
	...
	acceptedAuths := make([]*Auth, 0, len(auths))
	for _, auth := range auths {
		if auth == nil {
			continue
		}
		authID := strings.TrimSpace(auth.ID)
		if authID == "" || s.isStaleScheduledAuth(authID, auth.RegistrationEpoch, auth.Generation, auth.UpdatedAt) {
			continue
		}
		acceptedAuths = append(acceptedAuths, auth)
	}
	if s.scopedPool != nil {
		s.scopedPool.Rebuild(acceptedAuths)
	}
	now := time.Now()
	for _, auth := range acceptedAuths {
...
// RecordRemovalTombstone records a removal tombstone with the specified epoch and cleans up scheduler state.
...
func (s *authScheduler) pickMixedWithStrategy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, string, error) {
	...
	predicate = combineScheduledPredicates(predicate, s.scopedPoolMixedPredicateLocked(normalized))
...
func (s *authScheduler) scopedPoolPredicateLocked(providerKey string, shard *modelScheduler) func(*scheduledAuth) bool {
	...
	filtered := s.scopedPool.FilterCandidates(providerKey, shard.authsLocked())
	if len(filtered) == 0 {
...
func (s *authScheduler) scopedPoolMixedPredicateLocked(providers []string) func(*scheduledAuth) bool {
	if s == nil || s.scopedPool == nil || len(providers) == 0 {
		return nil
	}
	allowedProviders := make(map[string]struct{}, len(providers))
	for _, provider := range providers {
		allowedProviders[strings.ToLower(strings.TrimSpace(provider))] = struct{}{}
	}
	return func(entry *scheduledAuth) bool {
		if entry == nil || entry.auth == nil {
			return false
		}
		providerKey := strings.ToLower(strings.TrimSpace(entry.meta.providerKey))
		if _, ok := allowedProviders[providerKey]; !ok {
			return false
		}
		return len(s.scopedPool.FilterCandidates(providerKey, []*Auth{entry.auth})) > 0
	}
}
