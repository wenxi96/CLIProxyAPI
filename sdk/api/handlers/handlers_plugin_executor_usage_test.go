...
func TestHandlerPluginExecutorPublishesUsageNonStreamClaude(t *testing.T) {
	...
	if record.Detail.InputTokens != 50 || record.Detail.OutputTokens != 30 || record.Detail.ReasoningTokens != 10 || record.Detail.TotalTokens != 0 {
		t.Errorf("record.Detail = %+v, want input=50 output=30 reasoning=10 provider_total=0", record.Detail)
	}
	if breakdown := record.Detail.TokenBreakdown; !breakdown.Valid() || breakdown.TotalTokens != 80 || breakdown.Output.NonReasoningTokens != 20 || breakdown.Output.ReasoningTokens != 10 {
		t.Errorf("record.Detail.TokenBreakdown = %+v, want derived total=80 with reasoning split", breakdown)
	}
}
